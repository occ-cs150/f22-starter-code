...
extern std::vector<short> ans1;
extern std::vector<int> ans2;
extern std::vector<long> ans3;
extern std::vector<float> ans4;
extern std::vector<double> ans5;
...
/**
 * For generic types. Called from a macro.
 * @param msg - usually the stringified expression used to calculate "actual"
 * @param expected - expected string
 * @param actual - actual string.
 */
template <typename T>
void _assertEquals(const std::string& msg, const std::vector<T>& expected, const std::vector<T>& actual)
{
    extern int possible;
    possible++;
	std::string e = to_string(expected);
